  ...
  module Adapters
    ...
    class Slack < Adapter
      ...
      def channel_for(target)
        # Add condition (target.user.mention_name != robot.mention_name) to support message from bot
        if target.private_message? && target.user.mention_name != robot.mention_name
